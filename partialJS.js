...
function displayAll() {
    ...
    targetContainer.append(companies.forEach(giveCompHTML))
}

function giveCompHTML(companyObj) {
        ...
        compImage.src = companyObj['logo'];
        compName.textContent = companyObj['name'];
        compBlurb.textContent = companyObj['blurb'];
...
document.getElementById('addComp').addEventListener('click', () => {
    let addedComp = new Company(prompt('Enter Company Name:'), prompt('Enter Company Logo URL:'), prompt('Any Notes About The Company:'));
    document.getElementById('compList').append(giveCompHTML(addedComp));
    companies.push(addedComp)
})
